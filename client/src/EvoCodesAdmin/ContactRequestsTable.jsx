...
export default function ContactRequestsTable({ isDarkMode }) {
    ...
    const fetchContactRequests = async () => {
      try {
        ...
        const response = await axios.get(
          import.meta.env.VITE_BACKEND_API + "/contact-requests",
          {
  ...
  const handleBatchStatus = async (status) => {
    ...
    try {
      ...
      setRequests((prev) =>
        prev.map((r) =>
          selectedIds.includes(r.contactRequestId)
            ? { ...r, contactRequestStatus: status }
            : r,
        ),
      );
      setSelectedIds([]);
  ...
  const handleBatchDelete = async () => {
    ...
    if (confirm(`Delete ${selectedIds.length} selected request(s)?`)) {
      try {
        ...
        setRequests((prev) =>
          prev.filter((r) => !selectedIds.includes(r.contactRequestId)),
        );
        setSelectedIds([]);
      } catch (error) {
  ...
  const handleUpdateStatusSingle = async (id, newStatus) => {
    try {
      ...
      setRequests((prev) =>
        prev.map((r) =>
          r.contactRequestId === id
            ? { ...r, contactRequestStatus: updatedStatus }
            : r,
        ),
      );
      ...
      if (activeModalRequest && activeModalRequest.contactRequestId === id) {
        setActiveModalRequest((prev) => ({
          ...prev,
          contactRequestStatus: updatedStatus,
        }));
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap gap-2 text-xs font-semibold">
          <button
            ...
            className={`flex items-center gap-1.5 border px-4 py-2 rounded-lg transition-all cursor-pointer ${
              isDarkMode
                ? "bg-[#151c30] border-[#222f54] text-gray-300 hover:bg-[#1d2744]"
            ...
            <button
              ...
              className={`px-4 py-1.5 rounded-full font-medium transition-all cursor-pointer ${
                subFilter === tab
                  ...
                  : isDarkMode
                    ? "bg-[#151c30] text-gray-400 hover:text-white"
      ...
      <div
        className={`rounded-xl border overflow-hidden transition-colors w-full ${
          isDarkMode
            ? "bg-[#0f1422] border-[#1e2640]"
      ...
      >
        <div className="overflow-x-auto w-full">
          <table className="w-full border-collapse text-left min-w-[750px]">
            <thead>
              <tr
                className={`border-b text-[10px] uppercase font-bold tracking-wider text-gray-500 ${
                  isDarkMode
                    ? "bg-[#131a2e] border-[#1e2640]"
            ...
            <tbody
              className={`divide-y text-sm ${isDarkMode ? "divide-[#1e2640]/50" : "divide-gray-200"}`}
            >
              {loading ? (
                <tr>
                  <td
                  ...
                  >
                    <div className="flex justify-center items-center gap-2">
                      <Loader2
                        className="animate-spin text-[#4cc9f0]"
                        size={18}
                      />
                    ...
                    >
                      ...
                      <td className="py-4 px-6 flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <div
                            className={`font-bold transition-colors truncate ${isDarkMode ? "text-white" : "text-gray-900"}`}
                          >
                            {req.contactRequestSenderName}
                          </div>
                          <div className="text-xs text-gray-500 mt-0.5 truncate">
                            {req.contactRequestEmail}
                          </div>
                        </div>
                      </td>
                      <td
                        className={`py-4 px-6 font-medium max-w-[280px] truncate ${isDarkMode ? "text-gray-300" : "text-gray-700"}`}
                      >
                      ...
                      <td className="py-4 px-6">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded text-[10px] font-bold ${
                            req.contactRequestStatus === "NEW"
                              ? "bg-cyan-500/15 text-[#4cc9f0]"
                              : req.contactRequestStatus === "REPLIED"
                      ...
                      >
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            onClick={(e) =>
                              handleDeleteSingle(req.contactRequestId, e)
                            }
        ...
        {/* Dynamic Pagination */}
        <div
          className={`p-4 flex flex-col sm:flex-row gap-4 items-center justify-between text-xs text-gray-500 border-t ${
            isDarkMode
              ? "bg-[#131a2e]/60 border-[#1e2640]"
              : "bg-gray-50 border-gray-200"
          }`}
        >
          <div>
          ...
          <div className="flex gap-1">
            <button className="p-1.5 rounded border border-[#222f54] text-gray-500 hover:text-white cursor-pointer">
              <ChevronLeft size={14} />
            </button>
            <button className="px-2.5 py-1 rounded font-bold bg-[#4cc9f0] text-[#0b0f17]">
              1
            </button>
            <button className="p-1.5 rounded border border-[#222f54] text-gray-500 hover:text-white cursor-pointer">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div
            className={`w-full max-w-lg rounded-2xl border p-6 shadow-2xl relative ${
              isDarkMode
                ? "bg-[#0f1422] border-[#1e2640] text-white"
          ...
          >
            ...
            <div className="space-y-4 my-6 text-sm">
              <div>
                ...
                <p className="font-semibold text-base mt-0.5">
                  {activeModalRequest.contactRequestSenderName}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="font-medium text-[#4cc9f0] mt-0.5">
                  {activeModalRequest.contactRequestEmail}
                </p>
              </div>
              ...
              <div>
                ...
                <p className="font-medium mt-0.5">
                  {activeModalRequest.contactRequestSubject}
                </p>
              </div>
              ...
              <div>
                ...
                <div
                  className={`mt-1 p-3 rounded-xl border text-sm max-h-40 overflow-y-auto ${
                    isDarkMode
                      ? "bg-[#151c30] border-[#222f54] text-gray-300"
                      : "bg-gray-50 border-gray-200 text-gray-700"
                  }`}
                >
                  {activeModalRequest.contactRequestDesc ||
                    activeModalRequest.contactRequestMessage ||
                    activeModalRequest.message ||
              ...
              <div className="flex items-center justify-between pt-2">
                <div>
                  ...
                  <span className="text-xs text-gray-400 font-medium">
                    {formatDate(
                      activeModalRequest.contactRequestDate ||
                        activeModalRequest.createdAt,
                    )}
                  </span>
                </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-gray-700/30">
              <button
                onClick={(e) =>
                  handleDeleteSingle(activeModalRequest.contactRequestId, e)
                }
